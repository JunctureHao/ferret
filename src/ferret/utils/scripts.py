...
* **lupdate 不能 `-recursive` 整个包。** `core/resources_rc.py` 是 rcc 生成的几 MB 单文件
  ...
  （STATUS_STACK_BUFFER_OVERRUN）直接崩掉，连一行报错都不留 —— 历史上翻译目录
...
* **源语言是简体中文。** 代码里的界面文案一律直接写中文，英文由 `en_GB.ts` 提供。
  新增文案后必须重跑这条流水线，否则选英文的界面上就是原样的中文源文本
  （`tr()` 查不到时的行为）。

模块级的字面量表只能用 `QT_TRANSLATE_NOOP` 做标记、到使用点再
`QCoreApplication.translate` 求值：`application.py` 顶层就 import 了 `MainWindow`，
等 `_init_i18n()` 装翻译器时所有界面模块早已导入完毕，模块级求值会永久冻结成中文。
...
TS_DIR = I18N_DIR / "en_GB.ts"
QM_DIR = I18N_DIR / "en_GB.qm"
