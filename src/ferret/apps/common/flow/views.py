...
class OverviewTree(TreeWidget):
    ...
    显示哪些字段、怎么分组、什么时候整组藏起来，全在 `fields.SECTIONS` 里；这个类只剩
    「怎么把一组标签和值变成 `QTreeWidgetItem`」这一件事。加字段不用再碰这里。
    """
    ...
    def __init_widget(self):
        ...
        # 分组标题的两种字体：普通分组加粗，次级小节（证书的主体/签发者）改下划线。
        # 建在这里而不是模块级 —— `QFont()` 早于 QApplication 构造会告警。
        self._title_font = QFont()
        self._title_font.setBold(True)
        self._subgroup_font = QFont()
        self._subgroup_font.setUnderline(True)

    def set_data(self, data: dict):
        """按 `fields.SECTIONS` 填充树，只显示有值的字段
        ...
        for section in SECTIONS:
            self.addTopLevelItems(self._build_section(section, data))

    def _build_section(self, section: Section, data: dict) -> list[QTreeWidgetItem]:
        """建出一个分组，返回要挂到上一层的节点。

        三条规则都在这里，规格表只声明不实现：

        * 整组一行都显示不出来时返回空列表 —— 连标题节点都不建，不会剩下一个空标题；
        * 没有标题的分组把字段原样交回上一层，也就是最外层那批平级字段；
        * `when` 说不显示就整组不显示，哪怕组里有值。
        """
        ...
        for entry in section.fields:
            if isinstance(entry, Section):
                rows.extend(self._build_section(entry, data))
                continue
            value = field_value(entry, data)
            if value is not None:
                rows.append(self._build_field(entry, value, section.subgroup))
        ...
        node.setFont(0, self._subgroup_font if section.subgroup else self._title_font)
        node.addChildren(rows)
        return [node]

    def _build_field(self, field: Field, value: str, bullet: bool) -> QTreeWidgetItem:
        """一行「标签 / 值」；`bullet` 是证书那两个小节的 ``- `` 前缀。"""
        label = field_label(field)
        item = QTreeWidgetItem([f"- {label}" if bullet else label, value])
        for column in (0, 1):
            item.setTextAlignment(
                column, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter
            )
        return item
