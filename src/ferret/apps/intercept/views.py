...
class InterceptInterface(QWidget):
    ...
    def __build_rule_bar(self, parent: QWidget) -> QWidget:
        ...
        self.queue_btn = PushButton(FluentIcon.PAUSE_BOLD, self.tr("拦截队列"), bar)
        self.queue_btn.setToolTip(self.tr("打开断点窗口，处理已被拦下的流量"))
        ...
        self.enable_switch.setToolTip(
            self.tr("断点总开关。关闭后不再拦下新流量，已拦下的仍需手动处理")
        )
    ...
    def __build_rules_empty_page(self, parent: QWidget) -> QWidget:
        ...
        label = BodyLabel(self.tr("暂无断点规则"), page)
        hint = CaptionLabel(
            self.tr("规则可以在请求发出前、响应回来后或两个阶段都拦下流量"),
            page,
        )
