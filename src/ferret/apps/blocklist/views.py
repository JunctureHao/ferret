"""Block-rule interface: toolbar + rule table."""
...
from ferret.apps.blocklist.dialogs import BlockRuleDialog
...
class BlockListInterface(QWidget):
    """屏蔽规则页：命中的请求由 mitmproxy 原生 BlockList addon 直接回空响应。"""

    def __init__(self, controller: BlockListController, parent=None):
        super().__init__(parent)
        self.setObjectName("BlockListInterface")
    ...
    def __init_widget(self):
        ...
        self.source_model = BlockRuleTableModel(self)
        self.proxy_model = BlockRuleFilterProxyModel(self)
    ...
    def __build_empty_page(self) -> QWidget:
        ...
        label = BodyLabel(self.tr("暂无屏蔽规则"), page)
        hint = CaptionLabel(
            self.tr("命中规则的请求不会发往服务器，直接返回指定状态码"), page
        )
    ...
    @Slot()
    def _on_add(self):
        dialog = BlockRuleDialog(self.tr("新增屏蔽规则"), parent=self.window())
        if dialog.exec():
    ...
    def _on_edit(self):
        ...
        if rule is None:
            return
        dialog = BlockRuleDialog(
            self.tr("编辑屏蔽规则"), rule=rule, parent=self.window()
        )
        if dialog.exec():
