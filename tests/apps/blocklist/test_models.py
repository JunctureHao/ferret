...
def make_rule(value: str, **kwargs) -> BlockRule:
    return BlockRule(BlockField.HOST, BlockLogic.EQUALS, value, **kwargs)


class BlockRuleTableModelTests(unittest.TestCase):
    def setUp(self) -> None:
        self.model = BlockRuleTableModel()
        self.model.set_rules([make_rule("a.com"), make_rule("b.com", enabled=False)])
    ...
    def test_value_and_status_columns(self) -> None:
        role = Qt.ItemDataRole.DisplayRole
        self.assertEqual(self.model.data(self.model.index(0, 3), role), "a.com")
        self.assertEqual(
            self.model.data(self.model.index(0, 4), role), status_label(403)
        )

    def test_tooltip_shows_the_generated_expression(self) -> None:
        tooltip = self.model.data(self.model.index(0, 3), Qt.ItemDataRole.ToolTipRole)
        self.assertEqual(tooltip, r"~d ^a\.com$")

    def test_user_role_returns_the_rule(self) -> None:
        rule = self.model.data(self.model.index(1, 0), Qt.ItemDataRole.UserRole)
        self.assertEqual(rule, make_rule("b.com", enabled=False))
...
class BlockRuleFilterProxyModelTests(unittest.TestCase):
    def setUp(self) -> None:
        self.source = BlockRuleTableModel()
        self.source.set_rules(
            [
                make_rule("ads.example.com"),
                BlockRule(BlockField.METHOD, BlockLogic.CONTAINS, "POST"),
            ]
        )
        self.proxy = BlockRuleFilterProxyModel()
...
class BlockListControllerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.controller = BlockListController(mitm=MitmFacade(self.runtime))
    ...
    def test_starts_empty_and_pushes_to_the_runtime(self) -> None:
        ...
        self.assertEqual(self.runtime.block_rules, [])

    def test_add_rule_persists_and_pushes_down(self) -> None:
        rule = make_rule("a.com")
        self.assertTrue(self.controller.add_rule(rule))
        self.assertEqual(self.controller.rules, [rule])
        self.assertEqual(self.runtime.block_rules, [rule])
        self.assertEqual(CONFIG.get(CONFIG.block_list), [rule.to_dict()])

    def test_add_host_rule_dedups(self) -> None:
        failures: list[tuple[str, str]] = []
        self.controller.operation_failed.connect(
            lambda title, body: failures.append((title, body))
        )
        self.assertTrue(self.controller.add_host_rule("a.com"))
        self.assertFalse(self.controller.add_host_rule(" a.com "))
        self.assertEqual(len(self.controller.rules), 1)
        self.assertEqual(len(failures), 1)

    def test_add_host_rule_rejects_a_blank_host(self) -> None:
        self.assertFalse(self.controller.add_host_rule(""))
        self.assertEqual(self.controller.rules, [])

    def test_update_rule_replaces_in_place(self) -> None:
        self.controller.add_rule(make_rule("a.com"))
        self.assertTrue(
            self.controller.update_rule(0, make_rule("b.com", status_code=404))
        )
        self.assertEqual(self.controller.rules[0].value, "b.com")
        self.assertEqual(self.controller.rules[0].status_code, 404)

    def test_update_rule_out_of_range_is_a_noop(self) -> None:
        self.assertFalse(self.controller.update_rule(3, make_rule("a.com")))

    def test_remove_rules_drops_the_given_rows(self) -> None:
        for host in ("a.com", "b.com", "c.com"):
            self.controller.add_rule(make_rule(host))
        self.assertTrue(self.controller.remove_rules([0, 2, 99]))
        self.assertEqual([r.value for r in self.controller.rules], ["b.com"])
        self.assertEqual(len(CONFIG.get(CONFIG.block_list)), 1)
    ...
    def test_set_enabled_toggles_and_persists(self) -> None:
        ...
        self.assertFalse(self.runtime.block_rules[0].enabled)
        self.assertFalse(self.controller.set_enabled(0, False))

    def test_rules_changed_carries_a_copy(self) -> None:
        seen: list[list[BlockRule]] = []
        self.controller.rules_changed.connect(seen.append)
        self.controller.add_rule(make_rule("a.com", status_code=BLOCK_STATUS_CLOSE))
        self.assertEqual(len(seen), 1)
        seen[0].clear()
        self.assertEqual(len(self.controller.rules), 1)

    def test_unusable_persisted_rule_is_kept_but_disabled(self) -> None:
        broken = BlockRule(BlockField.HOST, BlockLogic.REGEX, "bad(")
        CONFIG.set(CONFIG.block_list, [broken.to_dict()])
        controller = BlockListController(mitm=MitmFacade(MitmRuntime()))
