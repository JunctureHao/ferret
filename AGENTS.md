...
- **block_private 为 wireguard 让路**：`_effective_block_private()` 在通道接通且 wireguard 开启时强制 False，用户配置值保留、回落即恢复。
...
- 不引入 `mitmproxy_rs` 的 `certs` / `syntax_highlight`；`rs_*`（local / wireguard / process_info）经 bindings 接入。
...
- 源语言是英文：所有 `tr()` / `translate()` 字面量写英文，中文只进 `resources/i18n/zh_CN.ts`；默认语言仍是简体中文；选 English 不装业务翻译器。
- 改任何文案后必须跑 `uv run python -m ferret.utils.scripts`（lupdate → lrelease → rcc 一条链，rcc 输出必须落 `core/resources_rc.py`）。少跑一步不报错，界面静默退回英文——`tests/core/test_i18n.py` 是守卫，跑它验证。lupdate 必须排除 `core/resources_rc.py`（3.6 MB 生成物会让它崩）；该文件是生成物勿手改（rcc 会把 mtime 编进资源表，diff 大是正常）。
...
- 模块级与类体（含 `ClassVar`）不得求值翻译（翻译器那时还没装，求出的文案会永久冻结成英文）：存 `QT_TRANSLATE_NOOP("Ctx", "text")` 标记，到使用点用 `QCoreApplication.translate` / `resolve_marker` 求值（`utils/i18n.py`）。
