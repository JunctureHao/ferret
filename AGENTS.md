...
- `apps/`：`capture`/`certificate`/`common`/`session`/`settings`/`blocklist`/`rewrite`/`intercept`（断点页只留规则，规则表单可选阶段：请求/响应/请求和响应；队列与请求/响应编辑器在独立的非模态窗口 `intercept/window.py`，构造时 parent 必须为 None，否则 `qframelesswindow` 不补 `Qt.Window` 会退化成子控件。窗口上半是队列表：每行 请求方式 + URL + 行内「放行/丢弃」按钮组（`setIndexWidget` 挂进操作格，模型重置前先 `setIndexWidget(index, None)` 拆旧）；下半是 `SegmentedWidget` 的「请求/响应」两个标签——请求期停在响应标签显示占位提示，响应期的请求标签锁只读供查看，写回按阶段选编辑器与停在哪个标签无关）/`compose`（手工请求编辑页：method/URL/参数/请求头/请求体 → `MitmFacade.send_custom_request`，结果经 `MitmRuntime.compose_result` 信号回来直接消费 `build_flow_detail` 字典），**不直接 import mitmproxy 内部模块**。后台任务统一用 `apps/common/tasks.py::FunctionTask`。编辑类 UI 复用 `apps/common/edit/`（`ItemDualPanel`/`ToolPlainTextEdit`/`JsonDualPanel`），不新造编辑器。
