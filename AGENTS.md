...
- **三通道抓包**（`core/mitm/modes.py`）：regular（系统代理底盘，监听恒在）+ local（本地重定向）+ wireguard（隧道），任意组合并存，经 `options.update(mode=[...])` 热更（官方路径：`proxyserver` configure 检测 mode 变更 → `Servers.update()` diff 热启停实例）。**不做 transparent**：Windows 上游明文 "unsupported, flaky, deprecated"（`platform/windows.py:468`，仅 warning 不拦）、需整个进程管理员（WinDivert）、重定向目标硬编码 8080（spec `@port` 传不进 redirector，改端口会全网断连）、随包分发 2015 年的 WinDivert 1.3.0。tun 在 Windows 不可用（Rust 侧 Linux-only）。
