...
- `core/mitm/`：`bindings`(唯一 mitmproxy 入口)/`master`/`runtime`/`facade`/`modes`(三通道 spec 组装与校验 + WireGuard 客户端配置/QR 矩阵生成，QR 编码用 `segno`——纯 Python 零二级依赖，走常规 import 图无需 Nuitka 特殊处理，见 §5)/`addons`(`FerretTlsConfig`/`LogAddon`)/`export`/`io`/`certificate`(同步阻塞，调用方负责挪后台线程)/`blocklist`/`rewrite`/`intercept`(断点规则+报文写回，规则可选阶段：请求/响应/两者，`intercept_expression` 按 phase 分组后用显式 `&` 挂 `~q`/`~s`——不能用并列，flowfilter 里并列优先级低于 `|`，会把整串段攥住)/`compose`(手工发送：`build_compose_flow` 从零造 flow 走 `ClientPlayback`，`ComposeAddon` 在 response/error 钩子里报结果并按 `record` 决定是否从 View 摘除)/`sse`(解析层纯函数零依赖 + `FerretSseAddon` tee，见 §6)/`__init__`(公开 API)。`engine.py` 零引用可删。**会送到幕界面的异常文案**（`certificate`/`facade`/`gateway`/`intercept`/`modes`/`rewrite`/`runtime`）用 `QCoreApplication.translate("<Ctx>", ...)` 包一层，所以这几个模块 import QtCore（不碰控件）；日志与 `from_dict` 校验消息不译（后者被 `rules_from_raw` 吞掉，从不上界面）。
...
- **守护进程拆除必须同步**（`MitmRuntime.stop` → `_disarm_local_redirector` + `_run_master` 启动防御）：守护进程是**进程外**服务，正常解除靠 `Servers.update` 的异步 stop 任务，但内核停止/重启会丢掉挂起任务——届时守护进程带旧 spec 继续截流（症状：关了本地重定向还能抓到本机流量），且残留的 `_instance` 占位会让新 Master 的 `_start` 被 "Cannot spawn more than one local redirector" 护栏拒掉（local 静默失效）。所以线程关闭前在**存活的事件循环上**同步 `set_intercept("")` + 清 `_instance`，每次 `_run_master` 开场再防御性清一次；`_server` 是跨 Master 的进程级类属性，测试经注入桩（替换 `LocalRedirectorInstance._server`）验证拆除时机，不碰真守护进程。
